...
from mylar_shield import MylarShield
...
class ActorCriticShielded(nn.Module):
    ...
    def act(self, state, deterministic=False):
        ...
        # log sensor values and action taken by the shielded policy
        if self.training:
            self.mylar_logs[self.time_step] = {
                    "sensor_values": sensor_values.cpu().numpy(),
    ...
    def evaluate(self, state, action):
        ...
        if self.shield is None or not self.shield.differentiable:
            ...
            sensor_values = self.shield.get_sensor_values(state)
            # compute the shielded policy
            # print("a",base_actions.shape, "s",sensor_values.shape, "st ",state.shape)
...
class MylarPPO:
    ...
    def sync_time_steps(self):
        """ Sync the time step between policy and agent. """
        self.policy.time_step = self.time_step
        self.policy_old.time_step = self.time_step

    def update_reward(self, reward, done, cost=0):
        self.buffer.rewards.append(reward)
        self.buffer.is_terminals.append(done)
        
        self.sync_time_steps()

        if not self.eval_mode:
            self.policy_old.mylar_logs[self.time_step]["safety cost"] = cost
            self.policy_old.mylar_logs[self.time_step]["done"] = int(done)
